...
| `add` | **添加资源**：生成并增量更新 harness 配置文件，或添加凭证（credential）。 | 配置可部署 harness 的参数；注册供 Agent 鉴权用的 API Key。 |
| `list` | **列举资源**：列出以 harness 方式部署的运行时，或列出凭证。 | 查看已部署的 harness 运行时；查看已注册的凭证。 |
| `delete` | **删除资源**：按名称删除凭证（credential）。 | 清理不再使用的 API Key 凭证。 |
...
| 选项 | 描述 | 示例 |
| :--- | :--- | :--- |
| `--template`, `-t` | 选择项目模板，如 `basic`、`basic_stream`、`a2a`、`harness`。`harness` 创建一个以 `project_name` 命名的目录，内含 `.env.example`（填入火山引擎 AK/SK）和 `Dockerfile`。 | `--template basic` |
...
# 示例 7：使用 harness 模板，在当前目录下创建 my_harness 目录（含 .env.example、Dockerfile）
agentkit init my_harness --template harness
```

#### 包装模式

```bash
# 示例 8：包装现有 Agent 文件（自动检测 Agent 变量）
agentkit init --from-agent ./my_agent.py

# 示例 9：包装并指定项目名称
agentkit init weather_bot --from-agent ./weather_agent.py

# 示例 10：使用简写和指定 Agent 变量名
...
`agentkit add` 用于添加并配置 Agent 资源。当前提供 `harness` 子命令，用于生成一个描述可部署 Agent 的 harness 配置文件 `<name>.harness.json`。

该文件以分层 JSON 描述 harness 的属性（模型、工具、技能、系统提示词、运行时，以及知识库 / 记忆组件和可选的 OAuth2 鉴权块），可被后续部署流程消费。

### 命令格式

```bash
agentkit add harness --name <name> [options]
```

文件写入到 `--directory`（默认当前目录）下的 `<name>.harness.json`。对同一 `--name` 重复执行会**增量合并**到现有文件，便于逐步完善配置。
...
| `--registry-endpoint` | A2A registry OpenAPI endpoint；运行时需要走公网 OpenAPI 网关或无法解析 region 专属域名时可设为 `https://open.volcengineapi.com/`。 | `--registry-endpoint https://open.volcengineapi.com/` |
| `--registry-region` | A2A registry OpenAPI region。 | `--registry-region cn-beijing` |
| `--register-self` | 注册当前项目已部署的 harness，从 `harness.json[name]` 读取 `url` 和 `runtime_id`。 | `--register-self` |
| `--register-space-id` | `--register-self` 使用的 A2A SpaceId。 | `--register-space-id as-xxx` |
| `--register-space-name` | `--register-self` 使用的 A2A Space 名称，CLI 会调用 `ListA2aSpaces` 解析为 SpaceId。 | `--register-space-name my-space` |
...
| 组件 | 类型选项 | 可选后端 | 连接参数（示例） |
| :--- | :--- | :--- | :--- |
| 知识库 | `--knowledgebase-type` | `viking` / `opensearch` / `redis` / `tos_vector` / `context_search` | `--knowledgebase-project`、`--knowledgebase-region`、`--knowledgebase-host`、`--knowledgebase-port`、`--knowledgebase-username`、`--knowledgebase-password`、`--knowledgebase-use-ssl`、`--knowledgebase-cert-path`、`--knowledgebase-secret-token`、`--knowledgebase-db` |
| 长期记忆 | `--long-term-memory-type` | `viking` / `opensearch` / `redis` / `mem0` | `--long-term-memory-project`、`--long-term-memory-region`、`--long-term-memory-host`、`--long-term-memory-port`、`--long-term-memory-username`、`--long-term-memory-password`、`--long-term-memory-db`、`--long-term-memory-api-key`、`--long-term-memory-api-key-id`、`--long-term-memory-project-id`、`--long-term-memory-base-url` |
| 短期记忆 | `--short-term-memory-type` | `local`（默认）/ `sqlite` / `mysql` / `postgresql` | `--short-term-memory-host`、`--short-term-memory-port`、`--short-term-memory-user`、`--short-term-memory-password`、`--short-term-memory-database`、`--short-term-memory-charset` |

### 鉴权参数（可选）

提供任一鉴权参数即启用 OAuth2/JWT 鉴权块。

| 选项 | 描述 |
| :--- | :--- |
| `--discovery-url` | OIDC discovery URL。 |
| `--allowed-id` | 允许的 client ID（逗号分隔）。 |

### 使用示例

```bash
# 示例 1：创建一个带模型、工具和系统提示词的 harness 配置
agentkit add harness --name my-harness \
  --model-name doubao-seed-1-6-250615 \
  --tools web_search,web_fetch \
  --system-prompt "You are a helpful assistant."

# 示例 2：配置 Viking 知识库（增量合并到已有文件）
agentkit add harness --name my-harness \
  --knowledgebase-type viking \
  --knowledgebase-project my-project \
  --knowledgebase-region cn-beijing

# 示例 3：启用 OAuth2/JWT 鉴权
agentkit add harness --name my-harness \
  --discovery-url "https://userpool-xxx.userpool.auth.id.cn-beijing.volces.com/.well-known/openid-configuration" \
  --allowed-id client-id-1,client-id-2

# 示例 4：注册当前项目已部署的 harness
agentkit add harness --name my-harness \
  --register-self \
  --register-space-id as-xxx \
  --register-network-type public

# 示例 5：开启 A2A Registry，并增强多轮工具调用稳定性
agentkit add harness --name my-harness \
...
# 示例 6：关闭已配置的 A2A Registry
agentkit add harness --name my-harness --registry disabled
```

执行示例 1、2 后生成的 `my-harness.harness.json`：
...
### agentkit add credential

`agentkit add credential` 注册一条供 Agent 鉴权使用的凭证。`--type api-key` 会调用 `CreateInboundAuthConfig`（`AuthType=ApiKey`），将 `--api-key` 的值以 `--name` 命名注册。

命令通过环境变量读取火山引擎凭证（AK/SK），无需显式传参。

#### 命令格式

```bash
agentkit add credential --type api-key --name <name> --api-key <api-key> [--region <region>]
```
...
| 参数 | 说明 | 示例 |
| :--- | :--- | :--- |
| `--type` | **（必填）** 凭证类型，当前支持 `api-key`。 | `--type api-key` |
| `--name` | **（必填）** 凭证名称，同时用作 API Key 名称（`ApiKeyName`）。 | `--name my-openai-key` |
| `--api-key` | API Key 值；`--type api-key` 时必填。 | `--api-key $OPENAI_API_KEY` |
| `--region` | 区域覆盖。默认取 `VOLCENGINE_AGENTKIT_REGION` / `VOLCENGINE_REGION` / 全局配置。 | `--region cn-beijing` |

#### 使用示例

```bash
# 注册一条名为 my-openai-key 的 API Key 凭证
agentkit add credential \
  --type api-key \
  --name my-openai-key \
...
`agentkit list` 用于列举 AgentKit 资源。当前提供 `harness` 子命令。

`agentkit list harness` 会使用环境变量中的火山引擎凭证（AK/SK）**列出所有可见的 AgentKit 运行时**，再筛选出在部署时被打上 harness 标签（`agentkit:agenttype=harness`）的运行时并展示。该标签由 `agentkit deploy --harness` 在创建运行时时自动写入。

> 列举过程会自动按 `NextToken` 游标翻页，遍历全部运行时后再做筛选，因此位于任意分页上的 harness 运行时都不会遗漏。

### 命令格式

```bash
agentkit list harness [options]
```

### 凭证

命令通过环境变量读取火山引擎凭证，无需显式传参：

- `VOLCENGINE_ACCESS_KEY` / `VOLCENGINE_SECRET_KEY`（或已导出的 `VOLC_ACCESSKEY` / `VOLC_SECRETKEY`）。

### 参数

| 参数 | 说明 | 示例 |
| :--- | :--- | :--- |
| `--region` | 区域覆盖（如 `cn-beijing`、`cn-shanghai`）。默认取 `VOLCENGINE_AGENTKIT_REGION` / `VOLCENGINE_REGION` / 全局配置。 | `--region cn-beijing` |
| `--output` | 输出格式：`table`（默认）、`json`、`yaml`。 | `--output json` |
| `--quiet`, `-q` | 仅打印 `RuntimeId`，便于脚本消费。 | `--quiet` |
| `--no-color`, `-nc` | 关闭表格/面板的彩色输出。 | `--no-color` |
| `--limit`, `-l` | 翻页时每批拉取的运行时数量（默认 `50`）。命令始终遍历全部分页。 | `--limit 100` |

### 使用示例

```bash
# 示例 1：以表格列出所有 harness 运行时
agentkit list harness

# 示例 2：仅打印 RuntimeId（适合脚本）
agentkit list harness --quiet

# 示例 3：以 JSON 输出，并指定区域
agentkit list harness --output json --region cn-beijing
```

### agentkit list credentials

`agentkit list credentials` 调用 `ListInboundAuthConfigs` 列出当前凭证可见的全部凭证（inbound auth config）。命令会自动按 `NextToken` 游标翻页。

#### 命令格式

```bash
agentkit list credentials [options]
```
...
| `--region` | 区域覆盖。默认取 `VOLCENGINE_AGENTKIT_REGION` / `VOLCENGINE_REGION` / 全局配置。 | `--region cn-beijing` |
| `--output` | 输出格式：`table`（默认）、`json`、`yaml`。 | `--output json` |
| `--quiet`, `-q` | 仅打印凭证名称，便于脚本消费。 | `--quiet` |
| `--no-color`, `-nc` | 关闭表格/面板的彩色输出。 | `--no-color` |
| `--limit`, `-l` | 翻页时每批拉取的数量（默认 `50`）。命令始终遍历全部分页。 | `--limit 100` |
...
`agentkit delete` 用于删除 AgentKit 资源。当前提供 `credential` 子命令。

`agentkit delete credential <name>` 先通过 `ListInboundAuthConfigs` 将名称解析为 `InboundAuthConfigId`，再调用 `DeleteInboundAuthConfig` 删除。若找不到对应名称的凭证，命令以非零退出码失败。

### 命令格式

```bash
agentkit delete credential <name> [--region <region>]
```
...
| 参数 | 说明 | 示例 |
| :--- | :--- | :--- |
| `<name>` | **（必填）** 要删除的凭证名称。 | `my-openai-key` |
| `--region` | 区域覆盖。默认取 `VOLCENGINE_AGENTKIT_REGION` / `VOLCENGINE_REGION` / 全局配置。 | `--region cn-beijing` |
...
`--harness` Harness 名称
- 指定后，改为部署**当前目录**下的 harness 规范文件 `<name>.harness.json`（云端构建 + 部署，一步完成），不再使用 `agentkit.yaml`
- 当前目录需同时包含该 `<name>.harness.json` 与 harness `Dockerfile`
- Runtime 名称即为该 `<name>`
...
### Harness 部署模式（`--harness`）

`<name>.harness.json` 是一份分层的 harness 配置规范（model / tools / skills /
knowledgebase / memory 等）。`agentkit deploy --harness <name>` 会：

1. 读取 `<path>/<name>.harness.json`，将其展平为 Runtime 环境变量；
2. 构建云端 AgentKit 启动配置并执行**构建 + 部署**（cloud build，无需本地 Docker）；
3. 为 Runtime 打上 `agentkit:agenttype=harness` 标签；
4. 成功后将 `{name: {url, key, runtime_id}}` 记录到同目录的 `harness.json` 注册表，便于按名称调用。

```bash
# 在 harness 目录下执行，部署当前目录的 mybot.harness.json
cd ./harness
agentkit deploy --harness mybot

# 指定区域
agentkit deploy --harness mybot --region cn-beijing
```

> 当前目录需同时包含 harness 服务的 `Dockerfile`（云端构建会原样使用它）。
...
```bash
# 调用 agentkit.yaml 配置的已部署 Agent（默认，run 可省略）
agentkit invoke [消息内容] [选项]

# 调用已部署的 harness（子命令）
agentkit invoke harness <name> [消息内容] [选项]
```

`invoke` 现在是一个命令组，含两个子命令：

- `run`（默认）：按 `agentkit.yaml` 调用已部署 Agent。直接 `agentkit invoke "你好"` 等价于 `agentkit invoke run "你好"`。
- `harness`：按名称调用已部署的 harness，从 `harness.json` 注册表解析运行时地址。
...
`--config-file` 配置文件路径（`run` 子命令）
- 指定配置文件位置
- 默认：当前目录的 `agentkit.yaml`

以上选项用于默认的 `run` 子命令。下面是 `harness` 子命令的专属参数。

### `agentkit invoke harness` 参数

**位置参数**

- `<name>`（必填）：已部署的 harness 名称，在 `--directory` 下的 `harness.json` 注册表中按名查找。
- `[消息内容]`：要发送的文字，自动包装成 <code v-pre>{"prompt": "..."}</code>。

**选项**

`--directory` 注册表所在目录
- 在该目录下查找 `harness.json` 注册表
- 默认：当前目录

`--region` AgentKit 区域
- 默认：环境变量 `VOLCENGINE_REGION` 或 `cn-beijing`

`--system-prompt` / `--model-name` / `--tools` / `--skills` / `--runtime` 单次调用覆写参数
- 镜像 `<name>.harness.json` spec 的 agent 字段，作为本次调用的 override 写入请求体的 `overrides` 字段
- `--tools` / `--skills` 接受逗号分隔列表
...
`--payload` / `--headers` / `--apikey` / `--raw` / `--show-reasoning`
- 与默认 `run` 子命令含义一致

### harness 调用模式

`agentkit deploy --harness <name>` 部署成功后，会把运行时地址记录到部署目录下的
`harness.json` **注册表**（详见 `agentkit deploy` 一节）。`agentkit invoke harness <name>`
按名在该注册表中查找，得到 endpoint / API Key（或 JWT 鉴权信息）来调用：

```json
{
  "my-harness": {
    "url": "https://xxx.apigateway-cn-beijing.volceapi.com",
    "key": "your_api_key",
    "runtime_id": "r-xxxxxx"
  }
}
...
- 在 `--directory`（默认当前目录）下查找 `harness.json`；找不到该名称时快速失败，并提示先用 `agentkit deploy --harness <name>` 部署。
- `custom_jwt` 鉴权的 harness 记录为 `{url, runtime_id, auth_type, discovery_url, allowed_ids}`（不含 key），调用时需通过 `--headers '{"Authorization": "Bearer <token>"}'` 提供用户 JWT。
- 覆写参数会作为 `overrides` 一并发送，例如：
...
#### 示例 8：调用已部署的 harness 并覆写系统提示词

```bash
agentkit invoke harness my-harness --system-prompt "请简洁作答。" "2+2=?"
```
